...
默认运行模式是：单用户、本地文件、localhost CLI/API、私有知识库。它不是多用户 SaaS，也不是公开 Web 服务。当前及后续阶段只面向本地单用户工程助手，优先保证语料质量、检索可靠性、证据可追溯、中英工程可用性和本地可复现性。

绑定产品纪律：共享、远程、公开部署和多用户能力无限期 defer。它们不属于 Phase 5 或任何已规划后续阶段的候选、entry gate 或隐含扩展方向；开发目标不得为其预留身份授权、租户隔离、公网入口、远程密钥、分布式限流或多用户审计实现。只有用户先显式修订本核心设计中的产品定位，才能另行提出新的阶段规划；普通 objective、技术评审或部署便利性不能自动解除该纪律。
...
- 投入共享、远程、公开或多用户部署加固；这些方向已无限期 defer。
